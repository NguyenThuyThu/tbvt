<?php 
	...
	class Mthanhtoan extends MY_Model
	{
		...
		public function xulydonhang($masp, $matv, $data){
			$this->db->where("tbl_ct_phieunhap.ma_sanpham", $masp);
			$row = $this->db->get("tbl_ct_phieunhap")->row_array();
			if($row['soluong_nhap']>0){
				$this->db->where("tbl_ct_phieunhap.ma_sanpham", $masp);
				$this->db->set("tbl_ct_phieunhap.soluong_nhap", $row['soluong_nhap'] - 1);
				$this->db->update("tbl_ct_phieunhap");
